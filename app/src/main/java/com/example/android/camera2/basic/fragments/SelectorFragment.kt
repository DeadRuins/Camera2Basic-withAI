...
class SelectorFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // 1. Get the list of all available cameras/formats
        val cameraList = enumerateCameras(cameraManager)

        // 2. Find the specific item: Back camera + JPEG format
        val selectedItem = cameraList.firstOrNull {
            it.title.contains("Back") && it.format == ImageFormat.JPEG
        } ?: cameraList.firstOrNull() // Fallback to the first available if Back/JPEG isn't found

        // 3. Navigate immediately if we found a camera
